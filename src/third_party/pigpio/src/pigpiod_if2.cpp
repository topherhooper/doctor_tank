   ...
   int pi, unsigned user_gpio, unsigned edge, void *f, void *user, int ex)
{
   ...
   if ((user_gpio >=0) && (user_gpio < 32) && (edge >=0) && (edge <= 2) && f)
   {
      ...
      p = static_cast<callback_t*>(malloc(sizeof(callback_t)));
   ...
   int pi, unsigned event, void *f, void *user, int ex)
{
   ...
   if ((event >=0) && (event < 32) && f)
   {
      ...
      ep = static_cast<evtCallback_t*>(malloc(sizeof(evtCallback_t)));
...
char const *pigpio_error(int errnum)
{
   ...
   if (gPigCommand[pi] >= 0)
   {
      ...
      if (gPigNotify[pi] >= 0)
      {
         ...
         else
         {
            ...
            userdata = static_cast<int*>(malloc(sizeof(*userdata)));
            *userdata = pi;
...
int callback(int pi, unsigned user_gpio, unsigned edge, CBFunc_t f)
   {return intCallback(pi, user_gpio, edge, reinterpret_cast<void*>(f), 0, 0);}

int callback_ex(
   int pi, unsigned user_gpio, unsigned edge, CBFuncEx_t f, void *user)
   {return intCallback(pi, user_gpio, edge, reinterpret_cast<void*>(f), user, 1);}
...
int event_callback(int pi, unsigned event, evtCBFunc_t f)
   {return intEventCallback(pi, event, reinterpret_cast<void*>(f), 0, 0);}

int event_callback_ex(
   int pi, unsigned event, evtCBFuncEx_t f, void *user)
   {return intEventCallback(pi, event, reinterpret_cast<void*>(f), user, 1);}
