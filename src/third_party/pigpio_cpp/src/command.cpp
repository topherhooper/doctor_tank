...
static char const * fmtMdeStr="RW540123";
static char const * fmtPudStr="ODU";
   ...
   switch (cmdInfo[idx].vt)
   {
      ...
      case 125: /* M MODES
         ...
         if ((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (f >= 1))
         {
            ctl->eaten += n;
            val = toupper(c);
            auto _p8 = strchr(fmtMdeStr, val); 
            memcpy(p8, _p8, strlen(_p8)+1);
...
int cmdParseScript(char *script, cmdScript_t *s, int diags)
{
   ...
   s->par = static_cast<int*>(calloc(1, b));
